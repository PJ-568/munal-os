...
use applib::{FbView, Framebuffer, OwnedPixels};
use chrono::{DateTime, Datelike, Month, Timelike, Utc};
...
pub fn topbar<'a, F: FbViewMut>(
    ...
    draw_rect(*fb, &topbar_rect, stylesheet.colors.background, false);
    ...
    let draw_monitor =
        |uitk_context: &mut uitk::UiContext<F>, x: &mut i64, monitor: &ResourceMonitor| {
            *x += ICON_MARGIN_W1 as i64;
            ...
            uitk_context
                .fb
                .copy_from_fb(monitor.icon, (icon_rect.x0, icon_rect.y0), true);
    ...
    let draw_text_box = |uitk_context: &mut uitk::UiContext<F>, x: &mut i64, text: &str, w: u32| {
        ...
        draw_rect(
            uitk_context.fb,
            &fps_rect,
            uitk_context.stylesheet.colors.element,
            false,
        );
        draw_line_in_rect(
            uitk_context.fb,
    ...
    let agg_frametime = frametime_data
        .iter()
    ...
    draw_text_box(
        uitk_context,
        &mut x,
        &format!("{:.0} FPS", 1000.0 / agg_frametime),
        FPS_COUNTER_W,
    );
    ...
    let agg_allocated = heap_allocated_data
        .iter()
        .fold(0.0, |acc, v| acc + v / heap_allocated_data.len() as f32);
