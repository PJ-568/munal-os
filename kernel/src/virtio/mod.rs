...
use crate::memory;
use crate::pci::{PciBar, PciConfigSpace, PciDevice};
        ...
        let desc_indices = {
            ...
            for i in 0..N {
                match self.take_descriptor() {
                    ...
                    None => {
                        // We couldn't reserve the required number of descriptors
...
impl VirtioDevice {
    pub fn new(pci_device: PciDevice, feature_bits: u32) -> Self {
            ...
            pci_device
                ...
                .filter_map(|pci_cap| {
                    let virtio_cap = unsafe {
                        pci_config_space
                            .read_struct::<VirtioPciCap>(&pci_device.addr, pci_cap.offset)
                    };
